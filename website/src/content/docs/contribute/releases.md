...
The **Package installation** workflow runs after each release upload and can also be dispatched with a published version. It exercises WinGet validation, unattended installation, command registration, removal and reinstallation on a disposable Windows runner. An empty version selects the latest public release. Its `catalog-submission-<attempt>` artifact contains the tested metadata; retries retain distinct artifacts. This workflow enables local manifests only on that runner and restores the setting afterward. A first release cannot demonstrate an upgrade from an earlier native release; test that separately when the next version is available.
