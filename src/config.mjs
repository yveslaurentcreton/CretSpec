...
async function validateGuidelines(directory) {
  ...
  if (!principles.isFile() || !profiles.isDirectory()) {
    throw new Error('Choose a guidelines repository containing guidelines/principles.md and profiles/.');
  }
  return root;
}
...
async function readConfig() {
  ...
  try {
    ...
    if (data?.schemaVersion !== 1 || typeof data.guidelinesRoot !== 'string' || !path.isAbsolute(data.guidelinesRoot)) {
      throw new Error('Invalid CretSpec configuration.');
    }
    return data;
  } catch (error) {
...
export async function getGuidelines() {
  ...
  if (!config) throw new Error('Configure your editable guidelines clone first: cspec guidelines set <path-to-CretAI>');
  try { return await validateGuidelines(config.guidelinesRoot); }
  catch (error) {
    throw new Error(`The configured guidelines directory is unavailable: ${config.guidelinesRoot}\n${error.message}\nRun cspec guidelines set <new-path> after moving or renaming the directory.`);
  }
}
