...
import { git, normalizeRepository, resolveRepository, resolveSpecSource, cloneProject, attachProject, workspacePath, projectInfo } from '../src/workspace.mjs';
...
async function fixture(t) {
  ...
  return { root, destination, guidelines, code, spec, manifest, lock, hash };
}
...
test('short names use the configured guidelines namespace, while explicit paths stay local', async t => {
  const f = await fixture(t);
  assert.equal(resolveSpecSource('Sample-spec', f.guidelines), f.spec);
  git(['remote', 'add', 'origin', 'git@github.com:owner/CretAI.git'], f.guidelines);
  assert.equal(resolveSpecSource('Sample-spec', f.guidelines), 'git@github.com:owner/Sample-spec');
  git(['remote', 'set-url', 'origin', 'https://github.com/owner/CretAI.git'], f.guidelines);
  assert.equal(resolveSpecSource('Sample-spec', f.guidelines), 'https://github.com/owner/Sample-spec');
  assert.equal(resolveSpecSource('./Sample-spec', f.guidelines, f.destination), path.join(f.destination, 'Sample-spec'));
  assert.equal(resolveSpecSource(f.spec, f.guidelines), f.spec);
});

test('one-argument clone creates sibling repositories and no enclosing project definition', async t => {
  const f = await fixture(t);
  await fs.writeFile(path.join(f.guidelines, 'uncommitted.md'), 'Private draft');
  const info = await cloneProject('Sample-spec', undefined, { guidelinesRoot: f.guidelines, cwd: f.destination });
  assert.equal(info.spec, path.join(f.destination, 'Sample-spec'));
  assert.equal(info.code, path.join(f.destination, 'Sample'));
  assert.deepEqual((await fs.readdir(f.destination)).sort(), ['Sample', 'Sample-spec']);
...
test('editor files are disposable, stay in the spec and read the current manifest', async t => {
  const f = await fixture(t);
  const options = { guidelinesRoot: f.guidelines, cwd: f.destination };
  ...
  assert.equal(path.resolve(path.dirname(filename), workspace.folders[2].path), f.guidelines);
  ...
  const newCode = path.join(f.destination, 'RenamedSample');
  ...
  assert.deepEqual((await fs.readdir(f.destination)).sort(), ['RenamedSample', 'Sample-spec']);
});

test('moving both repositories together needs no project registry or rewritten definition', async t => {
  const f = await fixture(t);
  const info = await cloneProject(f.spec, undefined, { guidelinesRoot: f.guidelines, cwd: f.destination });
  await workspacePath(info.spec, { guidelinesRoot: f.guidelines });
  ...
  const current = await projectInfo(spec, { guidelinesRoot: f.guidelines });
  assert.equal(current.code, path.join(relocated, 'Sample'));
  const filename = await workspacePath(spec, { guidelinesRoot: f.guidelines });
  const workspace = JSON.parse(await fs.readFile(filename, 'utf8'));
  assert.equal(path.resolve(path.dirname(filename), workspace.folders[2].path), f.guidelines);
...
test('attach preserves uncommitted work and rejects a second source of path bindings', async t => {
  ...
  const info = await attachProject(code, spec, { guidelinesRoot: f.guidelines });
  assert.equal(info.spec, spec);
  assert.equal(await fs.readFile(path.join(code, 'README.md'), 'utf8'), '# User changes\n');
  assert.equal(git(['status', '--porcelain'], spec), '');
  await assert.rejects(attachProject(f.code, spec, { guidelinesRoot: f.guidelines }), /sibling directory/);
  assert.deepEqual((await fs.readdir(f.destination)).sort(), ['Sample', 'Sample-spec']);
});
...
test('invalid or executable manifest fields fail before any code clone', async t => {
  ...
  await assert.rejects(cloneProject(f.spec, undefined, { guidelinesRoot: f.guidelines, cwd: f.destination }), /requires exactly/);
...
test('English help and version need no configuration; setup errors explain the next step', async t => {
  ...
  assert.throws(() => run('guidelines', 'path'), error => {
    assert.match(String(error.stderr), /Configure.*cspec guidelines set/);
    return true;
  });
...
test('a packaged CLI outside Git supports the short clone command and spec discovery', async t => {
  ...
  const spec = path.join(f.destination, 'Sample-spec');
  const info = JSON.parse(run('project', 'info', spec));
  assert.equal(info.editableGuidelines, f.guidelines);
  assert.equal(info.code, path.join(f.destination, 'Sample'));
...
test('invalid guidelines selection preserves personal configuration and relocation is explicit', async t => {
  const f = await fixture(t);
  const run = runner(f);
  run('guidelines', 'set', f.guidelines);
  ...
  assert.throws(() => run('guidelines', 'path'), error => {
    assert.match(String(error.stderr), /moving or renaming/);
    return true;
  });
...
test('unrelated guidelines and tracked local files cannot substitute private context', async t => {
  ...
  await assert.rejects(attachProject(f.code, f.spec, { guidelinesRoot: unrelated }), /git fetch --tags/);
  ...
  await assert.rejects(attachProject(f.code, f.spec, { guidelinesRoot: f.guidelines }), /tracks .local/);
...
test('a spec rejects unrelated existing code at the expected path without changing it', async t => {
  ...
  await assert.rejects(projectInfo(spec, { guidelinesRoot: f.guidelines }), /does not match code.repository/);
